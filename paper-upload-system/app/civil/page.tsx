      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-amber-600 mb-2">
            🏗️ Civil Engineering – Question Papers
          </h1>
          <p className="text-slate-600">Browse Civil Engineering papers by year and semester</p>
        </div>

        <div className="space-y-8">
          {civilData.map((yearBlock, yearIndex) => (
                      ...
                      <div className="mb-5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {years.map((yr) => (
                            <Link
                              key={`w-${yr}`}
                              href={`/civil/papers?year=${yearIndex + 1}&sem=${actualSem}&season=Winter&paperYear=${yr}`}
                              className="px-3 py-2 bg-amber-100 text-amber-700 rounded hover:bg-amber-200 hover:shadow-md transition text-sm font-medium"
                            >
                              {yr}
                            </Link>
                          ))}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {years.map((yr) => (
                            <Link
                              key={`s-${yr}`}
                              href={`/civil/papers?year=${yearIndex + 1}&sem=${actualSem}&season=Summer&paperYear=${yr}`}
